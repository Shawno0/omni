(() => {
  ...
  root.OmniRendererModules.createWorkspaceListController = ({
    ...
    // Incremental rendering cap — paints the first N items synchronously,
    // then reveals "Show more" batches. Prevents UI stalls when a user has
    // hundreds of workspaces. Reset whenever filters/sort change.
    const WORKSPACE_PAGE_SIZE = 60;
    let visibleCount = WORKSPACE_PAGE_SIZE;

    const createWorkspaceItemElement = (workspace) => {
    ...
    const renderWorkspaceList = () => {
      ...
      const slice = filtered.slice(0, Math.min(visibleCount, total));

      // Build into a DocumentFragment so we touch the live DOM exactly once.
      const fragment = document.createDocumentFragment();
      for (const workspace of slice) {
        fragment.append(createWorkspaceItemElement(workspace));
      }

      if (total > slice.length) {
        const moreItem = document.createElement("li");
        moreItem.className = "workspace-item workspace-item-more";
        const moreBtn = document.createElement("button");
        moreBtn.className = "ghost";
        moreBtn.textContent = `Show ${Math.min(WORKSPACE_PAGE_SIZE, total - slice.length)} more (${total - slice.length} hidden)`;
        moreBtn.addEventListener("click", (event) => {
          event.stopPropagation();
          visibleCount += WORKSPACE_PAGE_SIZE;
          renderWorkspaceList();
        });
        moreItem.append(moreBtn);
        fragment.append(moreItem);
      }
    ...
    const bindFilters = () => {
      ...
      elements.workspaceSearch?.addEventListener("input", resetAndRender);
      elements.workspaceSort?.addEventListener("change", resetAndRender);
    };
